...
function process(sourceFile, sourceContent, params) {
  ...
  var contentWithMap =
    sourceContent +
    convert.fromObject(prependFileProtocol(params.absSourceMap)).toComment({multiline: true});

  // need to prepend file protocol to source as well to avoid problems with source map
  var reworked = rework(contentWithMap, {source: prependFileProtocol(sourceFile)})
    .use(reworkPlugin)
    .toString({
  ...
  if (params.outputSourceMap) {
    return {
      ...
      map    : removeFileProtocol(reworked.map)
    };
  }
   ...
   */
  function removeFileProtocol(candidate) {
    if (typeof candidate === 'string') {
      return candidate.replace(/^file:\/{2}/, '');
    } else if (candidate && (typeof candidate === 'object') && Array.isArray(candidate.sources)) {
      return Object.assign({}, candidate, {
        sources: candidate.sources.map(removeFileProtocol)
      });
    } else {
      throw new Error('expected string|object');
    }
    ...
    function eachDeclaration(declaration) {
      ...
      if (isValid) {
        ...
        var directory =
          ...
          removeFileProtocol(path.dirname(startPosOriginal.source));
        if (directory) {
