...
function findFile(options) {
  ...
  function base(startPath, uri, limit) {
    ...
    var attemptsMax = Math.max(0, options.attempts) || 1E+9;
    var messages = [];
    ...
    while (queue.length && attemptsMax > attemptsMade++) {
      ...
      flushMessages('FOUND');
      if (options.attempts === null && attemptsMade > 1) {
        var msg = 'resolve-url-loader had to recursively search ' +
          absoluteStart + ' in order to locate ' + fullPath + '. This is because the file was not ' +
          'at its expected location (either relative to the file containing the "url()" directive ' +
          'or relative to the original entry file). In the next major version of resolve-url-loader, ' +
          'paths other than these will not be considered by default.';
        loader.emitWarning(new Error(msg));
      }
      return basePath;
    }

    if (attemptsMax <= attemptsMade) {
      // interrupted by attemptsMade
