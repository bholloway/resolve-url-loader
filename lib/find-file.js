 ...
 * @param {string|Array.<string>} [limit] Optional directory or list thereof which constrain the file search
 * @returns {string|null} <code>null</code> where not found else the absolute path to the file
 */
function absolute(startPath, uri, directories) {
  var basePath = base(startPath, uri, directories);
...
function base(startPath, uri, directories) {
  ...
  if (absoluteStart) {
    ...
    do {
      ...
      isWorking = testWithinDirectories(absoluteStart) && testNotPackage(absoluteStart);
   ...
   */
  function testWithinDirectories(absolutePath) {
    return directories.some(eachDirectory);

    function eachDirectory(directory) {
...
/**
 * Test whether the given value is a string.
 * @param {*} value The value to test
 * @returns {boolean} True for string, else False
 */
function resolvePathString(value) {
  var resolved = (typeof value === 'string') && path.resolve(value);
