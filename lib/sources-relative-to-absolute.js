 ...
 * @param {Array.<string>} [directories] Optional directories to limit the search to
 * @returns {string} The base path common to all relative source URIs or the given path where all are absolute
 */
module.exports = function sourcesRelativeToAbsolute(sources, defaultBasePath, directories) {
  return sources
    ...
    // not absolute
      ...
      var location = value
        ...
        .replace(/^[\\\/]\./, '.'),        // remove erroneous leading slash on relative paths
          basePath = findFile.base(defaultBasePath, location, directories);
